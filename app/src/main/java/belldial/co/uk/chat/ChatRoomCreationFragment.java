...
import belldial.co.uk.contacts.SearchContactViewHolder;
import belldial.co.uk.contacts.SearchContactsAdapter;
import belldial.co.uk.settings.LinphonePreferences;
import belldial.co.uk.views.ContactSelectView;
import belldial.co.uk.views.LinphoneLinearLayoutManager;
...
//                        mSearchAdapter.searchContacts(newText);
    ...
    public void onItemClicked(int position) {
        ...
        if (createEncryptedChatRoom && !searchResult.hasCapability(Friend.Capability.LimeX3Dh)) {
            Log.w(
            ...
            return;
                ...
                && !searchResult.hasCapability(Friend.Capability.GroupChat)) {
            Log.w(
                    "[Chat Room Creation] Contact "
                            + searchResult.getFriend()
    ...
    private void setSecurityEnabled(boolean enabled) {
        ...
        if (enabled) {
            ...
            for (ContactAddress ca : mSearchAdapter.getContactsSelectedList()) {
                ...
                if (ca.getContact() != null && !ca.hasCapability(Friend.Capability.LimeX3Dh)) {
                    toToggle.add(ca);
                } else {
                    if (ca.getView() != null) {
