    ...
    public void onBindViewHolder(@NonNull SearchContactViewHolder holder, int position) {
        ...
        if (contact != null) {
            ...
            ContactAvatar.displayAvatar(
                    contact,
                    contact.hasFriendCapability(Friend.Capability.LimeX3Dh),
                    holder.avatarLayout);

            if ((!mIsOnlyOnePersonSelection
                            && !searchResult.hasCapability(Friend.Capability.GroupChat))
                    || (mSecurityEnabled
                            && !searchResult.hasCapability(Friend.Capability.LimeX3Dh))) {
...
//     ContactsManager.getInstance().getMagicSearch().getContactListFromFilter(search, mOnlySipContact ? domain : "");
