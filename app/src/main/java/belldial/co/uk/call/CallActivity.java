...
import belldial.co.uk.activities.DialerActivity;
...
public class CallActivity extends LinphoneGenericActivity
    ...
    private static class HideControlsRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (call != null && call.getCurrentParams().isVideoEnabled()) {
                CallActivity activity = mWeakCallActivity.get();
                if (activity != null)
                    activity.updateButtonsVisibility(false);
            }
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mStatsFragment = (CallStatsFragment) getFragmentManager().findFragmentById(R.id.call_stats_fragment);
        mStatsFragment.setDrawer(sideMenu, sideMenuContent);

        CallStatusBarFragment statusBarFragment = (CallStatusBarFragment) getFragmentManager()
                .findFragmentById(R.id.status_bar_fragment);
        ...
        mListener = new SimpleCoreListener() {
            ...
            public void onCallStateChanged(
                ...
                if (state == Call.State.End || state == Call.State.Released) {
                    ...
                    boolean showAcceptUpdateDialog = LinphoneManager.getCallManager()
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (mAudioManager.onKeyVolumeAdjust(keyCode))
            return true;
    ...
    public void onUserLeaveHint() {
        ...
        if (call == null)
            return;
        boolean videoEnabled = LinphonePreferences.instance().isVideoEnabled()
                && call.getCurrentParams().isVideoEnabled();
    ...
    private void updateButtons() {
        ...
        mCore.setMicEnabled(recordAudioPermissionGranted);
        mMicro.setSelected(!mCore.isMicEnabled());
        ...
        mVideo.setSelected(call != null && call.getCurrentParams().isVideoEnabled());
        mSwitchCamera.setVisibility(
                call != null && call.getCurrentParams().isVideoEnabled()
                        ? View.VISIBLE
                        : View.INVISIBLE);
        ...
        if (mCore.getCalls() != null) {
            for (Call c : mCore.getCalls()) {
                if (c.getConference() != null) {
                    conferenceSize = c.getConference().getParticipantCount();
                    break;
                }
            }
        }
        ...
        mConference.setEnabled(
                mCore.getCallsNb() > 1
                        && mCore.getCallsNb() > conferenceSize
                        && !mCore.soundResourcesLocked());
    }

    private void toggleMic() {
        mCore.setMicEnabled(!mCore.isMicEnabled());
        mMicro.setSelected(!mCore.isMicEnabled());
    }
    ...
    private void moveLocalPreview(MotionEvent motionEvent) {
        switch (motionEvent.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                RelativeLayout.LayoutParams lp = (RelativeLayout.LayoutParams) mLocalPreview.getLayoutParams();
                ...
                if (left < 0
                        ...
                        || top + height + statusBarHeight >= screenSize.y)
                    return;
    ...
    private void createTimerForDialog(long time) {
        mCallUpdateCountDownTimer = new CountDownTimer(time, 1000) {
    ...
    private void displayConferenceCall(final Call call) {
        LinearLayout conferenceCallView = (LinearLayout) LayoutInflater.from(this)
        ...
        LinphoneContact contact = ContactsManager.getInstance().findContactFromAddress(call.getRemoteAddress());
        if (contact != null) {
        ...
        ImageView removeFromConference = conferenceCallView.findViewById(R.id.remove_from_conference);
    ...
    private void displayPausedConference() {
        LinearLayout pausedConferenceView = (LinearLayout) LayoutInflater.from(this)
    ...
    private void displayPausedCall(final Call call) {
        LinearLayout callView = (LinearLayout) LayoutInflater.from(this).inflate(R.layout.call_inactive_row, null,
                false);
