package belldial.co.uk.settings;
...
import belldial.co.uk.purchase.Purchasable;
...
public class LinphonePreferences {
    ...
    // Accounts settings
    private Account getAccount(int n) {
        if (getLc() == null) return null;
        Account[] accounts = getLc().getAccountList();
        if (n < 0 || n >= accounts.length) return null;
        return accounts[n];
    }
    ...
    private AuthInfo getAuthInfo(int n) {
        ...
        Address addr = account.getParams().getIdentityAddress();
    ...
    public String getAccountDomain(int n) {
        Account account = getAccount(n);
        return (account != null) ? account.getParams().getDomain() : "";
    }

    public int getDefaultAccountIndex() {
        if (getLc() == null) return -1;
        Account defaultAccount = getLc().getDefaultAccount();
        if (defaultAccount == null) return -1;

        Account[] accounts = getLc().getAccountList();
        for (int i = 0; i < accounts.length; i++) {
            if (defaultAccount.getParams().getIdentityAddress().equals(accounts[i].getParams().getIdentityAddress())) {
                return i;
            }
        }
        return -1;
    ...
    public int getAccountCount() {
        if (getLc() == null || getLc().getAccountList() == null) return 0;
    ...
    public void setAccountEnabled(int n, boolean enabled) {
        ...
        // If default account is disabled, try to set another one as default account
        if (!enabled
                && getLc().getDefaultAccount() != null
                && getLc().getDefaultAccount()
                        .getParams()
                        .getIdentityAddress()
                        .equals(account.getParams().getIdentityAddress())) {
            int count = getLc().getAccountList().length;
            if (count > 1) {
    ...
    private boolean isAccountEnabled(int n) {
        return getAccount(n).getParams().isRegisterEnabled();
    }
    ...
    public void setIceEnabled(boolean enabled) {
        ...
        if (enabled) nat.setStunEnabled(true);
        getLc().setNatPolicy(nat);
    }
    ...
    public void setPushNotificationEnabled(boolean enable) {
        ...
        if (enable) {
            ...
            if (regId != null && core.getAccountList().length > 0) {
                for (Account account : core.getAccountList()) {
                    if (account == null) continue;
                    AccountParams params = account.getParams().clone();
                    if (!params.isPushNotificationAvailable()) {
                        params.setContactUriParameters(null);
                        account.setParams(params);
                        if (account.getParams().getIdentityAddress() != null)
                            Log.d(
                                    "[Push Notification] infos removed from account "
                        ...
                        String contactInfos =
                                "app-id="
                                        ...
                                        + regId
                                        + ";pn-silent=1";
                        String prevContactParams = params.getContactParameters();
                        if (prevContactParams == null
                                || prevContactParams.compareTo(contactInfos) != 0) {
                            params.setContactUriParameters(contactInfos);
                            account.setParams(params);
                            if (account.getParams().getIdentityAddress() != null)
                                Log.d(
                                        "[Push Notification] infos added to account "
                                                + account.getParams().getIdentityAddress().asStringUriOnly());
                        }
                    }
                }
    ...
    public String getPushNotificationRegistrationID() {
        return getConfig() == null
                ? null
                : getConfig().getString("app", "push_notification_regid", null);
    }
    ...
    public void setDebugEnabled(boolean enabled) {
        getConfig().setBool("app", "debug", true);
    ...
    public void enableTunnelDualMode(boolean enable) {
        LinphoneManager.getInstance().initTunnelFromConf();
        getLc().getTunnel().setDualModeEnabled(enable);
    }

    public boolean isTunnelDualModeEnabled() {
        Tunnel tunnel = getLc().getTunnel();
        if (tunnel != null) {
            return tunnel.isDualModeEnabled();
        }
        return false;
    }
    ...
    public String getActivityToLaunchOnIncomingReceived() {
        return getConfig()
                .getString(
                        ...
                        "org.linphone.belldial.co.uk.ui.activity.CallIncomingActivity");
    }
    ...
    public void enableDeviceRingtone(boolean enable) {
        getConfig().setBool("app", "device_ringtone", true);
