...
import belldial.co.uk.assistant.PhoneAccountLinkingAssistantActivity;
import belldial.co.uk.call.CallManager;
...
import belldial.co.uk.utils.MediaScanner;
...
public class LinphoneManager implements SensorEventListener {
    ...
    private AccountCreatorListener mAccountCreatorListener;
    ...
    public LinphoneManager(Context c) {
        ...
        mAccountCreatorListener = new AccountCreatorListener() {
            @Override
            ...
            @Override
            public void onAccountCreationTokenUsingRequestToken(
                    AccountCreator accountCreator,
                    AccountCreator.Status status,
                    String resp) {
                // Stub
            ...
            @Override
            public void onAccountCreationRequestToken(
                    AccountCreator accountCreator,
                    AccountCreator.Status status,
                    String resp) {
                // Stub
    ...
    public static synchronized LinphoneManager getInstance() {
        LinphoneManager manager = null;
        try {
            manager = LinphoneContext.instance().getLinphoneManager();
        } catch (Exception e) {
            android.util.Log.d("asad", "getInstance1111111: " + e.getMessage());
            e.printStackTrace();
        }
        android.util.Log.d("asad", "getInstance:2222333   " + LinphoneContext.instance().getLinphoneManager());
        try {
            manager = LinphoneContext.instance().getLinphoneManager();
            if (manager == null) {
                // throw new RuntimeException(
                // "[Manager] Linphone Manager should be created before
                // accessed");
            }
            if (manager.mExited) {
                // throw new RuntimeException(
            ...
            android.util.Log.d("asad", "getInstance:2222333   " + LinphoneContext.instance().getLinphoneManager());
            android.util.Log.d("asad", "getInstance:222222222 " + e.getMessage());
    ...
    public static synchronized Core getCore() {
        try {
            if (getInstance().mExited) {
                ...
                android.util.Log.d("asad", "getCore:datadataddat " + AppController.getInstance());

                LinphoneManager manager = new LinphoneManager(AppController.getInstance());
                return manager.mCore;
            ...
            e.printStackTrace();
            android.util.Log.d("asad", "getCore:ssssssss " + e.getMessage());
            return null;
        }
    ...
    public synchronized void destroy() {
        ...
        // Otherwise some objects might crash during their own destroy if they try to
        // call
    ...
    private void destroyCore() {
        ...
        if (LinphonePreferences.instance() != null) {
            // We set network reachable at false before destroy LC to not send register with
            // expires
            // at 0
    ...
    public synchronized void startLibLinphone(boolean isPush) {
        try {
            ...
            /*
             * use schedule instead of scheduleAtFixedRate to avoid iterate from being call
             * in burst after cpu wake up
             */
        ...
        // H264 codec Management - set to auto mode -> MediaCodec >= android 5.0 >=
        // OpenH264
    ...
    private synchronized void initLiblinphone(Core core) {
        ...
        // Migrate existing linphone accounts to have conference factory uri and LIME
        // X3Dh url set
        String uri = getString(R.string.default_conference_factory_uri);
        for (Account account : mCore.getAccountList()) {
            if (account.getParams().getIdentityAddress().getDomain().equals(getString(R.string.default_domain))) {
                if (account.getParams().getConferenceFactoryUri() == null) {
                    AccountParams params = account.getParams().clone();
                    Log.i(
                            "[Manager] Setting conference factory on account "
                                    + account.getParams().getIdentityAddress().asString()
                                    + " to default value: "
                                    + uri);
                    params.setConferenceFactoryUri(uri);
    ...
    public void isAccountWithAlias() {
        if (mCore.getDefaultAccount() != null) {
    ...
    private void askLinkWithPhoneNumber() {
        ...
        Account account = mCore.getDefaultAccount();
        if (account == null)
            return;
        if (!account.getParams().getDomain().equals(getString(R.string.default_domain)))
            return;

        long future = new Timestamp(
        ...
        final Dialog dialog = LinphoneUtils.getDialog(
                mContext,
                String.format(
                        getString(R.string.link_account_popup),
                        account.getParams().getIdentityAddress().asStringUriOnly()));
    ...
    public void changeStatusToOnThePhone() {
        ...
        if (isPresenceModelActivitySet()
                && mCore.getPresenceModel().getActivity().getType() != PresenceActivity.Type.OnThePhone) {
            mCore.getPresenceModel().getActivity().setType(PresenceActivity.Type.OnThePhone);
        } else if (!isPresenceModelActivitySet()) {
            PresenceModel model = mCore.createPresenceModelWithActivity(PresenceActivity.Type.OnThePhone, null);
            mCore.setPresenceModel(model);
        }
    }
    ...
    public void checkForUpdate() {
        ...
        if (url != null && !url.isEmpty()) {
            ...
            int interval = mContext.getResources().getInteger(R.integer.time_between_update_check); // 24h
